...
| `make-installer.ps1` | Runs `fetch-phonon.ps1` and its own `dotnet publish` (kept separate from `package.ps1`'s staging so the two can run independently), assembles the same `README.txt`/`SETUP.md`/`THIRD-PARTY-NOTICES.txt` alongside the publish output, renders `Images/Logo.png` down to a 100x100 `.bmp` (Inno's wizard image keys require `.bmp`, not `.png`), then compiles `installer.iss` via Inno Setup's `ISCC.exe` into a single setup `.exe`. |
| `installer.iss` | Inno Setup script for the installer: per-user install under `%LocalAppData%\Programs\LogiBuddy` (no admin/UAC required), Start Menu shortcut, optional desktop shortcut, standard uninstaller. Uses `Images/logibuddy-light.ico` directly as the installer's own icon. Takes `AppVersion`/`SourceDir`/`WizardSmallImage` from `make-installer.ps1` via `/D` defines. |
